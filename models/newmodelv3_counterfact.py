...
class NewModelv3_Counterfact(torch.nn.Module):
    ...
    def embedding(self, state, target, action_embedding_input, scene, target_object, counterfact):
        ...
        x = torch.cat((x, target_embedding, target_indicator_embedding, action_reshaped), dim=1)

        x = F.relu(self.pointwise(x))
        x = self.dropout(x)
        out = x.view(x.size(0), -1)
        counterfact_out = self.counterfact_embedding(target_embedding, action_reshaped)

        return out, image_embedding, action_at, counterfact_out

    def counterfact_embedding(self, target_embedding, action_reshaped):
        target_indicator_embedding = torch.zeros(1, self.num_cate, 7, 7).to(target_embedding.device)
        image_embedding = torch.zeros(1, 64, 7, 7).to(target_embedding.device)
        x = self.dropout(image_embedding)
        x = torch.cat((x, target_embedding, target_indicator_embedding, action_reshaped), dim=1)
    ...
    def forward(self, model_input, model_options, counterfact=False):
        ...
        x, image_embedding, action_at, x_counterfact = self.embedding(state, target, action_probs, scene, target_object, counterfact)
        actor_out_counterfact, _, _ = self.a3clstm(x_counterfact, hx, cx)
        actor_out, critic_out, (hx, cx) = self.a3clstm(x, hx, cx)
        lambda_a = 1.2
        actor_out = lambda_a*actor_out + (1-lambda_a)*actor_out_counterfact

        actor_out = torch.mul(actor_out, action_at)
        # if counterfact:
        #     print(self.object_distribution.kl[-1])
