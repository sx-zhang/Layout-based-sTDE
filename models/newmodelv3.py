...
class NewModelv3(torch.nn.Module):
    ...
    def embedding(self, state, target, action_embedding_input, scene, target_object, counterfact):
        ...
        if counterfact:
            target_indicator_embedding = torch.zeros(1, self.num_cate, 7, 7).to(target.device)
            image_embedding = torch.zeros(1, 64, 7, 7).to(target.device)
        else:
